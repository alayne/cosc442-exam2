...
public class DfaAttackAction extends DisplacementAttackAction {
    ...
    public static ToHitData toHit(IGame game, int attackerId,
        ...
        // Refactored out helper method here
        if (needToReachTarget(target, chargeStep)) {
        	return new ToHitData(TargetRoll.IMPOSSIBLE,
	...
	private static boolean needToReachTarget(Targetable target, MoveStep chargeStep) {
		if ((chargeStep == null)
    ...
    public static ToHitData toHit(IGame game, int attackerId,
        ...
        // Refactored out this to new checkFriendlyFireMethod
        if (checkFriendlyFire(game, target, ae)) {
        ...
        // Refactored out to new method checkElevation
        if (checkElevation(target, ae, targetElevation, attackerHeight)) {
        	return new ToHitData(TargetRoll.IMPOSSIBLE,
        ...
        //Refactored out to new method checkAnotherDisplacemetnAttack
        if (checkAnotherDisplacementAttack(ae, te)) {
        	return new ToHitData(TargetRoll.IMPOSSIBLE,
        ...
        // refactored out to new method
        if (checkAdjacentBuilding(target)) {
        ...
        // *** Refactored out this helper method ***
        toHit = isMech(ae, toHit);
	...
	private static boolean checkWoods(Targetable target) {
	...
	private static boolean checkAdjacentBuilding(Targetable target) {
	...
	private static boolean checkAnotherDisplacementAttack(final Entity ae, Entity te) {
	...
	private static boolean checkElevation(Targetable target, final Entity ae, final int targetElevation,
			final int attackerHeight) {
	...
	private static boolean checkFriendlyFire(IGame game, Targetable target, final Entity ae) {
	...
	private static ToHitData isMech(final Entity ae, ToHitData toHit) {
